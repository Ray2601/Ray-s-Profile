...
export function Hero() {
  ...
  return (
    <section id="home" className="home-section relative grid h-dvh snap-start snap-always grid-rows-[1fr_auto] overflow-hidden px-5 pb-[clamp(1.25rem,3vh,2rem)] pt-[calc(4rem+clamp(1.25rem,3vh,2rem))] md:px-8">
      <div
        aria-hidden
      ...
      <div className="mx-auto grid min-h-0 w-full max-w-6xl items-center gap-[clamp(2rem,5vw,5rem)] md:grid-cols-[1.1fr_0.9fr]">
        <div className="relative">
          ...
          <h1 className="mt-[clamp(1rem,2.5vh,1.75rem)] text-balance text-[2.65rem] font-medium leading-[1.12] tracking-[-.045em] md:text-[clamp(3.5rem,5vw,4.6rem)]">
          ...
          <p className="mt-[clamp(1rem,2.2vh,1.5rem)] max-w-xl text-pretty text-[15px] font-normal leading-7 text-muted-foreground md:text-[17px]">
            {profile.intro}
          </p>
        ...
        <div className="soft-float relative mx-auto h-[min(50vh,450px)] w-fit max-w-sm [--tilt:2deg]">
          <div className="absolute -inset-3 -z-10 rotate-[-5deg] rounded-[1.75rem] bg-[#dfeaf6]" />
          <div className="h-full rotate-2 overflow-hidden rounded-[1.5rem] border-[10px] border-white bg-white shadow-[0_18px_50px_rgba(93,75,60,.14)]">
            <Image
      ...
      <div className="mx-auto mt-[clamp(.75rem,2vh,1.5rem)] grid w-full max-w-6xl grid-cols-2 gap-3 md:grid-cols-4">
        {stats.map((s, i) => (
          <div key={s.label} className={`rounded-[1.35rem] px-5 py-[clamp(.7rem,1.8vh,1.25rem)] text-center ${['bg-[#f8ddd8]','bg-[#fff0bd]','bg-[#dfeef7]','bg-[#e8e0f2]'][i]}`}>
